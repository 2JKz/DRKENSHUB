local VenyxLibrary = loadstring(game:HttpGet'"https://raw.githubusercontent.com/2JKz/DRKENSHUB/main/GUIVENYX.lua'))()
...
local keys = {}
local function pflying(player, pflySpeed)
	...
	if pflyEnabled == true then
		if keys.w_active then
			velocity = velocity + GetVelocity(hrp.Position,(hrp.CFrame*frontoffset).Position,pflySpeed)
		end
		if keys.s_active then
			velocity = velocity + GetVelocity(hrp.Position,(hrp.CFrame*backoffset).Position,pflySpeed)
		end
		if keys.a_active then
			velocity = velocity + GetVelocity(hrp.Position,(hrp.CFrame*leftoffset).Position,pflySpeed)
		end
		if keys.d_active then
			velocity = velocity + GetVelocity(hrp.Position,(hrp.CFrame*rightoffset).Position,pflySpeed)
		end
		if keys.e_active then
			velocity = velocity + GetVelocity(hrp.Position,(CFrame.new(hrp.Position)*upoffset).Position,pflySpeed)
		end
		if keys.q_active then
			velocity = velocity + GetVelocity(hrp.Position,(CFrame.new(hrp.Position)*downoffset).Position,pflySpeed)
		end
		hrp.Velocity = velocity
		hrp.CFrame = CFrame.new(hrp.Position, (workspace.Camera.CFrame*(CFrame.new()+Vector3.new(0,0,-maxdistance))).Position)
		--	if not keys.w_active and not keys.a_active and not keys.s_active and not keys.d_active and not keys.q_active and not keys.e_active then
		--		hrp.Anchored = true
		--	else
		--		hrp.Anchored = false
...
local wsSpeed = 16
local wsSection = playerPage:addSection("Walkspeed")
...
hbSection:addSlider("Size", 1, 1, 20, function(v) hb = v / 1 end)
